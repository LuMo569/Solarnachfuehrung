...
void loop() {
  ...
  if(tol < -1*difver || tol < difver){    //s. o.
    if(oben < unten){                     //wenn der wert unten größer ist (ergo unten ist es heller) dann neige die Solarplatte nach unten (mit dem Servo)
      myservo.write(20);
      }else if(oben > unten){             //andernfalls nach oben
        myservo.write(-20);      
        }
    } 
